...
public class ActionController {
    ...
    public  static MyButton buttonAction( PrintWriter moveWriter, MyButton firstClicked, MyButton button, PegBoard boardState,  MyButton[][] buttonsMatrix, JLabel error_msg, Boolean randRunning){
        ...
        //if its the second button selected, the buttons will check if it's a valid move 
            ...
            logMove(moveWriter, source[0], source[1], goal[0], goal[1]);
    

           int randomInRange = (int)(Math.random() * (100+ 1));
           System.out.println(randomInRange);

           //50% chance only when running random
           if(randomInRange<50 && randRunning){
                BoardGUI.resetBoard(boardState.getSize(), boardState.getType(), true);
           }
    ...
    public static void autoPlayAction( PrintWriter moveWriter, PegBoard boardState,  MyButton[][] buttonsMatrix,JLabel error_msg, Boolean isRunningRandom){
        if(isRunningRandom){
            error_msg.setText("Reset board to run an Auto Game");
            return;
        
        }

        Timer timer = new Timer(300, null);
        logText(moveWriter, "Auto:");
